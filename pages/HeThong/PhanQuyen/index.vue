<template>
  <v-container fluid>
    ...
    <v-row class="mx-auto">
      <v-col cols="4">
        <DanhMucPhanQuyen
          ref="gridDanhMucPhanQuyen"
          @RowSelectChange="onChonPhanQuyen"
          :du-an-id="duAnId"
        />
      </v-col>
      <v-col cols="8">
        <DxTreeList
          ...
          id="treePhanQuyen"
          ref="treePhanQuyen"
          :data-source="dataMenu"
          :column-auto-width="true"
          :show-row-lines="true"
          :show-borders="true"
          :expanded-row-keys="expandedRowKeysMenu"
          :selected-row-keys.sync="selectedRowKeysMenu"
          key-expr="menu_id"
...
export default {
  ...
  methods: {
    getDanhSachMenuChon(data) {
      let dataReponse = []
      data.map(x => {
        dataReponse.push(x.menu_id);

        if (x.children) {
          dataReponse = dataReponse.concat(this.getDanhSachMenuChon(x.children));
        }
      });
      return dataReponse;
    },
    ...
    onSelectionDuAnChanged(e) {
      ...
      if (this.$refs.gridDanhMucPhanQuyen) {
        this.$refs.gridDanhMucPhanQuyen.GetDataPhanQuyen(this.duAnId);
      }
    },
    onChonPhanQuyen(e) {
    ...
    async GetDataPhanQuyen(duAnId, phanQuyenId) {
      ...
      if (this.dataMenu.length === 0 || this.dataBaoCao) {
        await this.GetData();
      }
      ...
      this.selectedRowKeysMenu = null;
      this.$services.phanquyenmenu
        .apiGetByPhanQuyenIdAndDuAnId(par)
        .then(async res => {
          if (res.status === 200) {
            if (res.data.data) {
              this.selectedRowKeysMenu = this.getDanhSachMenuChon(res.data.data);
            }
          }
        })
        .catch(err => {
    ...
    async SaveData() {
      ...
      const dataSave = {
        ...
        danh_sach_menu: this.selectedRowKeysMenu,
      }
      await this.$services.phanquyenmenu
        .apiInsert(dataSave)
        .then(res => {
          if (res.status === 200) {

            if (res.data.data) {
              this.selectedRowKeys = this.getDanhSachMenuChon(res.data.data);
            }
    ...
    async GetData() {
      ...
      await this.$services.menu
        ...
        .then(res => {
          this.dataMenu = res.data.data.map(x => {
            if (!x.parent_id) {
              x.parent_id = 0;
            }
            ;
          ...
          this.expandedRowKeysMenu = this.dataMenu.map(x => x.menu_id);
        })
        .catch(err => {
          console.log(err);
        });
      ...
      await this.$services.chucnang
        ...
        .then(res => {
          ...
          this.expandedRowKeysChucNang = this.dataMenu.map(x => x.chu_nang_id);
        })
        .catch(err => {
          console.log(err);
        });

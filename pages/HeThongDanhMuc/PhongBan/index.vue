  ...
  <v-container >
    ...
    >
      ...
      <dx-editing :allow-updating="true" :allow-deleting="true" mode="popup">
        <dx-popup :show-title="true" :width="700" :height="525" title="Phòng ban">
          <dx-position my="center" at="center"/>
        </dx-popup>
        <dx-form :form-data="itemUpdate">
          <DxItemForm :col-count="1" :col-span="2" item-type="group">
            <DxItemForm data-field="ten_phong_ban"/>
            <DxItemForm data-field="parent_key_id"/>
            <DxItemForm dataType="boolean" data-field="tinh_trang"/>
          </DxItemForm>
        </dx-form>
      </dx-editing>

      <dx-column alignment="center" :width="70" data-field="phong_ban_id" caption="ID"/>
      ...
      <dx-column data-field="parent_key_id" caption="cấp trên" alignment="center">
        <dx-lookup :data-source="dataPhongBanCombobox" value-expr="phong_ban_id" display-expr="ten_phong_ban"/>
      </dx-column>
    </dx-data-grid>
...
export default {
  ...
  created() {
    this.$store.commit("setTilePage", "Danh mục phòng ban");
  ...
  methods: {
    ...
    async GetDataCombobox() {
      await this.$services.phongban
        ...
        .then(res => {
          this.dataPhongBanCombobox = res.data.data;
        })
        .catch(err => {
          console.log(err);
        });
    ...
    async onRowRemoving(e) {
      if (e.cancel === false) {
        ...
        await this.$services.phongban
          .apiDeletebyId(dataRemove.phong_ban_id)
          .then(res => {

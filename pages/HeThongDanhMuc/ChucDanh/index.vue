      ...
      <dx-editing :allow-updating="true" :allow-deleting="true" mode="popup">
        ...
        <dx-form :form-data="itemUpdate">
          <DxItemForm :col-count="1" :col-span="2" item-type="group">
            <DxItemForm data-field="ten_chuc_danh"/>
            <DxItemForm data-field="ten_chuc_danh_viet_tat"/>
            <DxItemForm dataType="boolean" data-field="tinh_trang"/>
          </DxItemForm>
        </dx-form>
      </dx-editing>

      <dx-column alignment="center" :width="70" data-field="chuc_danh_id" caption="ID"/>
      <dx-column data-field="ten_chuc_danh" caption="Tên chức danh">
        <dx-required-rule/>
      </dx-column>

      <dx-column data-field="ten_chuc_danh_viet_tat" caption="Tên viết tắt"/>
...
export default {
  ...
  created() {
    this.$store.commit("setTilePage", "Danh mục chức danh");
  ...
  methods: {
    ...
    async onRowRemoving(e) {
      if (e.cancel === false) {
        ...
        await this.$services.chucdanh
          .apiDeletebyId(dataRemove.chuc_danh_id)
          .then(res => {

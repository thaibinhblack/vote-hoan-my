  ...
  <v-dialog v-model="dialog" persistent max-width="800">
    ...
    <v-card>
      <v-system-bar color="primary" lights-out window dark>
        <span class="font-weight-medium">Thêm mới Report</span>
        <v-spacer></v-spacer>
      ...
      <ValidationObserver ref="observer" v-slot="{ validate,invalid, reset }">
        <v-card-text>
          ...
          <form>
            <DuAnCombobox
              :read-only="false"
              :value.sync="itemEdit.du_an_id"
              label="Dự án"
              required
              :du-an-id="duAnId"
            />
            <v-text-field
              v-model="itemEdit.report_code"

              label="Report code"
              required
            />
            <ValidationProvider v-slot="{ errors }" name="Report name" rules="required">
              <v-text-field
            ...
            <ReportCombobox :value.sync="itemEdit.parent_id" label="Report cấp trên" :du-an-id="duAnId"/>
        ...
        <v-card-actions>
          ...
          <v-btn @click="saveData" color="error" class="ma-2">
            <v-icon :disabled="invalid" left>save</v-icon>
            Lưu
          </v-btn>
        </v-card-actions>
      </ValidationObserver>
...
import ReportCombobox from "~/components/Templates/Report/ReportCombobox";
...
export default {
  ...
  created() {
    ...
    console.log(this.duAnId);

    if (this.duAnId) {
      this.itemEdit.du_an_id = this.duAnId;
    }
  ...
  methods: {
    ...
    async saveData() {
      ...
      if (this.itemEdit.report_id) {
        this.$services.report.apiUpdate(this.itemEdit)
          .then(res => {
      ...
      // thêm mới
      if (!this.itemEdit.report_id) {
        this.$services.report.apiInsert(this.itemEdit)
          .then(res => {
